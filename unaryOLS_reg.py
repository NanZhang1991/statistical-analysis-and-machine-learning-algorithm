...
def unaryOLS(df):
    ...
    uOLS_df = pd.DataFrame(data = [[res.params[0], res.params[1], pre_y,] + res.conf_int().iloc[1].tolist()], 
                               index = ['unaryOLS_reg'], 
                               columns = ['intercept','x_coef','cf_lower_bound','cf_upper_bound','pre_y']) 
    return uOLS_df
...
#    local test 
    cmd = "python unaryOLS_reg.py --file_path ./test_data/input/unary_OLS.csv"